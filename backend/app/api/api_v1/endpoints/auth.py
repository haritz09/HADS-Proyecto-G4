...
from backend.app.db.schema import UserRead, UserBase
...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    # Convertir _id a str explícitamente
    if "_id" in user:
...
def login_user(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    update_user(str(user["_id"]), {"last_login": datetime.now(UTC)})
...
async def update_profile(update: UserProfileUpdate, current_user: dict = Depends(get_current_user)):
    ...
    if update_data:
        ...
        current_user = get_user(str(current_user["_id"]))
        if "_id" in current_user:
            current_user["_id"] = str(current_user["_id"])
    return current_user
